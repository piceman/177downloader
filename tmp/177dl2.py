...
# def getMaxPages(main_page):
#     # find each sub page
#     p = requests.get(main_page)
#     pagesoup = BeautifulSoup(p.text,'lxml')
#     title=pagesoup.title.text
#     print(title)
#     sub_pages = pagesoup.find(attrs={'class':'page-links'}).find_all('a') # 直接查找attrs判断页面
#     # print(sub_pages)
#     print('----')
#     max_page = 0
#     for x in sub_pages:
...
def downloadComic(comic_link):      # 下载图片
    ...
    for x in range(1,max_page+1):
        ...
        for y in tmp:
        #     match= re.search(r'(?:src=)([\w://.-]*)', y, re.I | re.S | re.M)
        #     if match:
        #         print(match.group(1))
        #         imglist.append(match.group(1))
            imglist.append(y)
    os.mkdir(title)
    os.chdir(title)
...
def main():
    # downloadComic('http://www.177pic.info/html/2015/05/55597.html')
    # downloadComic('http://www.177pic.info/html/2018/04/2044414.html')
    urls= []
